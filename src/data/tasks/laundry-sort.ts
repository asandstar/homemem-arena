...
// 保留历史容器 id，避免破坏旧存档与测试 API；公开语义已经改为无需开合的台面/开放架。
const PREP_COUNTER = 'cnt-cabinet-lower'
const OPEN_SHELF = 'cnt-cabinet-upper'
...
// 只使用模型真实支持的能力：静态家具承载可见物品，不伪装柜门/抽屉动画。
// 备餐台填补冰箱与现有工作台之间的空位；开放架放在灶台右侧的北墙空位。
const PREP_LOCAL = { x: -1.4, y: 0, z: -1.95 }
const SHELF_LOCAL = { x: 1.9, y: 0, z: -2.05 }
const SHELF_SURFACE_Y = 0.64
// applyScriptedMove 内部会自动叠加 room.center，所以这里传**房间局部坐标**
const SHELF_WORLD = {
  room: 'dining' as const,
  x: SHELF_LOCAL.x,
  y: SHELF_SURFACE_Y + 0.015,
  z: SHELF_LOCAL.z + 0.08,
}
...
function tableIsSet(ctx: StageContext): boolean {
  const spoon = ctx.entities.find((entity) => entity.configId === SPOON)
  const spoonAlreadySet = spoon?.surfaceContainerId === DINING_TABLE
    || entityPlacedIn(ctx.entities, SPOON, DINING_TABLE)
  return entityPlacedIn(ctx.entities, BOWL, DINING_TABLE)
    && entityPlacedIn(ctx.entities, CUP, DINING_TABLE)
    && spoonAlreadySet
}
...
function cerealMovedToShelf(ctx: StageContext): boolean {
  const cereal = ctx.entities.find((entity) => entity.configId === CEREAL)
  return cereal?.placedIn === OPEN_SHELF
    && cereal.status === 'placed'
    && cereal.properties?._moving !== true
}
...
export const laundrySortTask: TaskConfig = {
  ...
  // 五阶段包含记忆过期核对与更新；去掉与核心无关的餐后清理，降低评委完成成本。
  timeLimit: 600,
  ...
  stages: [
    {
      id: STAGE_ENCODE,
      playerObjective: '【形成记忆】北墙冰箱右侧的备餐台上有麦片、碗和杯子。靠近发光的麦片盒按 E，记住它现在的位置。',
      entryCondition: () => true,
    ...
    {
      id: STAGE_DISTRACTOR,
      playerObjective: '【分散注意】把备餐台上的碗和杯子放到餐桌。勺子已经摆好；先不要拿麦片。',
    ...
    {
      id: STAGE_STALE,
      playerObjective: '【旧记忆冲突】现在去取麦片。先按记忆回到原来的备餐台；如果那里空了，说明这条记忆已经过期。',
    ...
    {
      id: STAGE_UPDATE,
      playerObjective: '【重新观察并更新】旧位置已经失效。观察房间里的开放置物架，找到可见的麦片；靠近后按 E 更新记忆。',
...
这一关升级为 UPDATE：主人准备早餐时临时接到电话，请 MEM-07 帮忙摆桌。麦片、碗和杯子都清楚地放在北墙冰箱右侧的备餐台上；勺子已经在餐桌上（本关用的是勺子，不是叉子）。

这次真正考验的不是“找东西”，而是判断记忆是否仍然可信：
  ① 靠近备餐台上发光的麦片盒，按 E 保存位置记忆
  ② 把同一台面上的碗和杯子摆上餐桌；勺子已经摆好
  ③ 回原来的备餐台取麦片，确认旧位置已经变空
  ④ 重新观察，在开放置物架上找到麦片并按 E 更新记忆
  ⑤ 按更新后的记忆拿到麦片并放上餐桌

⚠️ 这不是隐藏物品搜索：冲突确认后，当前目标会引导你观察开放架。真正需要你完成的是判断旧记忆失效，并亲自按 E 更新。`,

  completionText: '✅ 早餐已经摆好！\nMEM-07：「旧记忆不是答案，只是一次有时间戳的观察。发现冲突、重新观察、更新后再行动——UPDATE 模块校准完成。」',
  failureText: '⏰ 时间到了，但这次失败不会清空你的思路。\n提示：先用 E 记住麦片；摆好碗杯后回备餐台确认冲突，再观察开放置物架并用 E 更新。',
  systemPrompt: '【MEM-07 日志】L3 UPDATE。唯一会移动的核心对象是麦片：备餐台 → 开放置物架。全程无需打开家具；必须先保存旧记忆，环境变化后发现冲突，再保存一次新记忆，才能完成。',
  ...
  objects: [
    {
      ...
      category: 'cereal',
      initialRoom: 'dining',
      initialPosition: { x: PREP_LOCAL.x, y: 0, z: -2.12 },
      surfaceContainerId: PREP_COUNTER,
    ...
    },
    {
      ...
      category: 'bowl',
      initialRoom: 'dining',
      initialPosition: { x: -1.56, y: 0, z: -2.02 },
      surfaceContainerId: PREP_COUNTER,
    ...
    },
    {
      ...
      category: 'cup',
      initialRoom: 'dining',
      initialPosition: { x: -1.24, y: 0, z: -2.02 },
      surfaceContainerId: PREP_COUNTER,
  ...
  containers: [
    ...
    },
    {
      ...
      name: '开放置物架',
      room: 'dining',
      position: SHELF_LOCAL,
      visualOffset: { x: 0, y: 0, z: -0.35 },
      size: { x: 0.64, y: 0.64, z: 0.4 },
      surfaceHeight: SHELF_SURFACE_Y,
      color: '#a16207',
      initialOpen: false,
      openable: false,
      acceptedCategories: [],
      acceptAny: false,
      targetLabel: '开放置物架（观察新位置）',
  ...
  goals: [
    ...
    },
    {
      ...
      description: '回到备餐台，发现旧记忆已过期',
      kind: 'milestone',
      dependsOnGoalIds: ['g-set-breakfast-table'],
      memoryType: 'temporal',
      relatedObjectIds: [CEREAL],
      relatedContainerIds: [PREP_COUNTER],
    ...
    },
    {
      ...
      memoryType: 'spatial',
      relatedObjectIds: [CEREAL],
      relatedContainerIds: [OPEN_SHELF],
  ...
  scriptedEvents: [
    {
      ...
      trigger: 1,
      type: 'message',
      message: '🥣 冰箱右侧备餐台上的物品都已可见。先靠近发光的麦片盒按 E；形成旧位置记忆后，再搬碗和杯子。',
      description: '提示玩家形成麦片的初始位置记忆',
      memoryType: 'object',
      toastType: 'info',
      highlightDemo: { targetContainerId: PREP_COUNTER, color: '#f59e0b', durationMs: 2600 },
    },
    {
      id: 'se-cereal-moved',
      ...
      type: 'move-entity',
      targetId: CEREAL,
      targetPosition: SHELF_WORLD,
      targetContainerId: OPEN_SHELF,
      markMemoryOutdated: CEREAL,
      message: '🐱 身后传来一声轻响。你关于麦片的记忆变红了——它可能已经不再可靠。',
      description: '麦片从备餐台被移动到开放置物架，旧记忆过期',
      memoryType: 'temporal',
      toastType: 'warning',
    ...
    },
    {
      ...
      trigger: (_step, _entities, _room, _rooms, ctx) => !!ctx
        ...
        && cerealMovedToShelf(ctx)
        && nearLocal(ctx, PREP_LOCAL),
      type: 'message',
      message: '⚠️ 备餐台上的麦片不见了。你没有记错——是这条记忆已经过期。重新观察房间，但系统不会直接标出答案。',
