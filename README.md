...
> 一款让你顺便练记忆的 3D 网页小游戏。你扮演记忆容量有限的家政机器人「小橡」，只有 3 个记忆槽，必须主动按 `E` 决定记住哪些物品的位置。三关递进地考验不同记忆能力：先学 ENCODE（编码），再学 RECALL（稳定回忆），最后在物品被悄悄移动后核对冲突、按 `E` 更新过期记忆（UPDATE）。
>
> 同时也是一个轻量研究平台：游戏结构化记录你的每一次观察、操作与记忆更新，用于研究家务机器人如何在动态环境中维护长期记忆。
...
- **记忆会过期（L3）**：旗舰关里物品会被悄悄移动一次，你之前保存的旧记忆随之失效——必须核对冲突、重新观察并按 `E` 更新。
